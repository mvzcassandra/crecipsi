...
def cargar():
    ...
    # Percentiles calculados del master limpio (4,184 registros)
    pp = pd.read_csv("percentiles_peso.csv")
    pt = pd.read_csv("percentiles_talla.csv")

    # Convertir CSV a dicts {stats_M: DataFrame, stats_H: DataFrame}
    sr = {
    ...
    # M2: Peso ~ edad_meses + talla_cm + sexo_num  → features: [edad, talla_cm, sexo]
    mp = joblib.load("modelo_peso_alzada.pkl")
    # M3: Talla ~ edad_meses + sexo_num            → features: [edad, sexo]
    ma = joblib.load("modelo_talla.pkl")
    # M1: Peso ~ edad_meses + sexo_num (fallback)  → features: [edad, sexo]
    mp1 = joblib.load("modelo_peso_simple.pkl")

    return sr, sa, mp, ma, mp1
...
with tab3:
    ...
    with sb3:
        ...
        if alz3>0:
            st.markdown('<div class="strip-ok">Modelo M2 · R²=0.9664</div>',
                        unsafe_allow_html=True)
        else:
            st.markdown('<div class="strip-info">Modelo M1 · R²=0.9540</div>',
                        unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with ct3:
        ...
        # Alzada de referencia en cm (P50 del mes)
        alz_m3= ref_a3["p50"].values[0] if len(ref_a3)>0 else 135.0
        alz_u3= alz3 if alz3>0 else alz_m3
        # M2: features [edad_meses, talla_cm, sexo_num]
        if alz3>0:
            peso3 = mod_peso.predict([[edad3, alz_u3, sbin3]])[0]
        else:
            # M1 fallback sin talla: features [edad_meses, sexo_num]
            peso3 = mod_peso_simple.predict([[edad3, sbin3]])[0]
        # M3: features [edad_meses, sexo_num] → cm
        ...
        # Gráfica predictor
        # Para cada mes: usar M2 si existe percentil de talla con n>=3,
        # si no usar M1 (sin talla) para evitar la "V" por fallback incorrecto
        ...
        pM3=[]; pH3=[]; aM3=[]; aH3=[]
        for e in edades_g:
            rm=saM3[saM3.edad_meses==e]; rh=saH3[saH3.edad_meses==e]
            # Macho
            if len(rm)>0 and rm["n"].values[0]>=3:
                am=rm["p50"].values[0]
                pM3.append(mod_peso.predict([[e,am,1]])[0])
                aM3.append(round(am,1))
            else:
                pM3.append(mod_peso_simple.predict([[e,1]])[0])
                aM3.append("—")
            # Hembra
            if len(rh)>0 and rh["n"].values[0]>=3:
                ah=rh["p50"].values[0]
                pH3.append(mod_peso.predict([[e,ah,0]])[0])
                aH3.append(round(ah,1))
            else:
                pH3.append(mod_peso_simple.predict([[e,0]])[0])
                aH3.append("—")
        ...
        ax3.legend(); ax3.set_xlim(-0.3,22.5); ax3.set_ylim(0,560)
        plt.tight_layout()
...
with tab5:
    ...
    with sb5:
        ...
        for mes in MESES_IA:
            ...
            with c2: av=st.number_input("cm",min_value=0.0,max_value=200.0,value=0.0,step=0.5,key=f"aia_{mes}")
...
with tab6:
    ...
    with ct6:
        ...
        for item in [
            "Percentiles P10–P90 por edad (0–23 meses) y sexo — calculados del master limpio (n=4,184), NumPy con interpolación lineal",
            "Regresión polinomial grado 3 — scikit-learn Pipeline(PolynomialFeatures, StandardScaler, LinearRegression) con scaler integrado",
            "M2 (principal): peso ~ edad + talla_cm + sexo; M1 (fallback): peso ~ edad + sexo; M3: talla ~ edad + sexo",
            "Validación cruzada 5-fold (KFold, random_state=42) — R² CV=0.9658, MAE CV=14.9 kg (M2)",
            "Master construido desde 11 archivos Excel con fecha de nacimiento exacta por fila — edad calculada con relativedelta",
            "Clasificador de patrón basado en criterios clínicos percentilados — calibrado para tasa irregular ~1.4%",
            "Correlación peso–talla: r=0.9664 (Pearson, p<0.001) — justifica inclusión de talla como covariable",
        ]:
            st.markdown(
