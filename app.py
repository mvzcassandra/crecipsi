...
<div class="metrics-row">
  ...
  <div class="metric-cell"><div class="metric-val">4,175</div><div class="metric-lbl">Mediciones</div></div>
  <div class="metric-cell"><div class="metric-val">10 años</div><div class="metric-lbl">2015 – 2025</div></div>
  <div class="metric-cell"><div class="metric-val">0.964</div><div class="metric-lbl">R² modelo</div></div>
  <div class="metric-cell"><div class="metric-val">±15 kg</div><div class="metric-lbl">Error medio</div></div>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    with ct1:
        ...
        with st.expander("Ver tabla completa de valores"):
            ...
            st.dataframe(t_full.round(1 if "Peso" in vr1 else 3),
...
with tab3:
    ...
    with sb3:
        ...
        if alz3>0:
            st.markdown('<div class="strip-ok">Modelo mejorado · R²=0.9641</div>',
                        unsafe_allow_html=True)
        else:
            st.markdown('<div class="strip-info">Sin alzada · R²=0.9458</div>',
                        unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with ct3:
            ...
            with c_res2:
                if len(ref_a3)>0:
                    ...
                    <div class="pred-box" style="background:var(--color-background-secondary)">
                        ...
                        <div class="pred-val" style="color:var(--color-text-primary)">{alzp3:.3f} m</div>
                        <div style="font-size:11px;color:var(--color-text-secondary);margin-top:6px">
                            Mediana rancho: {a50:.3f} m
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
        ...
        aM3=[saM3[saM3.edad_meses==e]["p50"].values[0] if len(saM3[saM3.edad_meses==e])>0 else 1.35 for e in edades_g]
        aH3=[saH3[saH3.edad_meses==e]["p50"].values[0] if len(saH3[saH3.edad_meses==e])>0 else 1.33 for e in edades_g]
        ...
        ax3.legend(); ax3.set_xlim(0.5,22.5)
        plt.tight_layout()
...
with tab6:
    ...
    with sb6:
        ...
        st.dataframe(pd.DataFrame({
            "Modelo":["Peso+alzada","Peso solo","Alzada"],
            "R2":["0.9641","0.9458","0.9552"],
            "MAE":["15.1 kg","19.6 kg","2.0 cm"],
        }), use_container_width=True, hide_index=True)

        st.markdown('<div class="section-lbl">Dataset</div>', unsafe_allow_html=True)
        for lbl,val in [("Animales","217"),("Mediciones peso","4,175"),("Mediciones alzada","3,981"),("Con peso al nacer","89.4%"),("Periodo","2015–2025"),("Machos / Hembras","111 / 106")]:
        ...
        for pat,n,pct in [("Normal",143,"65.9%"),("Superior",37,"17.1%"),("Inferior",34,"15.7%"),("Irregular",3,"1.4%")]:
    ...
    with ct6:
        ...
        for item in [
            "Percentiles P10–P90 por edad (0–22 meses) y sexo — función NumPy con interpolación lineal",
            "Regresión polinomial grado 3 — scikit-learn Pipeline(PolynomialFeatures, LinearRegression)",
            "Variables predictoras de peso: sexo + edad + alzada (Modelo v2) y sexo + edad (Modelo v1)",
            "Validación train/test 80%/20% con semilla fija (random_state=42) para reproducibilidad",
            "Clasificador de patrón basado en criterios clínicos percentilados — reglas interpretables",
            "Correlación peso–alzada: r=0.9666 (Pearson, p<0.001) — justifica uso conjunto en modelos",
        ]:
            st.markdown(
